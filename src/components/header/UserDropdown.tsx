...
export default function UserDropdown() {
  ...
  // Récupération du user et de la fonction logout depuis Zustand
  ...
  const handleLogout = (e: React.MouseEvent) => {
    ...
    router.push("/signin"); // Redirige vers la connexion
  };
    ...
    <div className="relative">
      ...
      >
        <div className="px-3 py-2 border-b border-gray-100 dark:border-gray-800 mb-2">
          ...
          <span className="mt-0.5 block text-[11px] font-medium text-gray-500 dark:text-gray-400 truncate">
            {user?.email || "Pas d'email"}
          </span>
          {/* Badge pour le rôle (ADMIN, USER, etc.) */}
          <span className="mt-2 inline-block px-2 py-0.5 text-[9px] font-black uppercase tracking-widest bg-emerald-50 text-[#00A09D] rounded-md">
            {user?.role || "Membre"}
          </span>
        </div>
        ...
        <ul className="flex flex-col gap-1 pb-3 border-b border-gray-200 dark:border-gray-800">
          ...
          {/* ... autres items (Settings, Support) ... */}
        </ul>

        <button
          onClick={handleLogout}
          className="flex items-center gap-3 px-3 py-2 mt-3 font-medium text-red-500 rounded-lg group text-theme-sm hover:bg-red-50 transition-colors w-full text-left"
        >
          <svg
